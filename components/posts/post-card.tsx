...
export async function PostCard({ post }: { post: Post }) {
  ...
  return (
    <Link
      href={`${post?.link.replace(process.env.WORDPRESS_URL ?? "", "")}`}
      className={cn(
      ...
      <div className="flex flex-col gap-4">
        ...
        <div
          ...
          className="text-xl text-primary font-medium group-hover:underline decoration-muted-foreground underline-offset-4 decoration-dotted transition-all"></div>
        <div
          className="text-sm"
          dangerouslySetInnerHTML={{
