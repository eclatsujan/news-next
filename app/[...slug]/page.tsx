import { getPostBySlug, getFeaturedMediaById, getAuthorById, getCategoryById, getAllPostSlugs, getPostByLink, getPostById } from "@/lib/wordpress";
...
export default async function Page({ params }: { params: Promise<{ slug: string[] | string }> }) {
  ...
  const post = await getPostById(Array.isArray(slug) ? parseInt(slug[slug.length - 1]) : parseInt(slug));
